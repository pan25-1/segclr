...
def add_contrastive_loss(
  ...
  # [a1, a2, a3, a4]    [b1, b2, b3, b4]   a1b1 b2b2... 表示同一个图像的两个增强版本，
  # 在SegCLR中a1b1表示从同一个segment上采样的两个cotout图像增强的结果 a2 a3 a4..为其他segment上采样的
  ...
  if strategy is not None:
    ...
    labels = tf.one_hot(tf.range(batch_size), batch_size * 2)  # 用来只计算postives的损失
    masks = tf.one_hot(tf.range(batch_size), batch_size)       # 将本身a1a1 b1b1视为-inf, 避免将同一样本与自身进行比较
  ...
  loss = tf.reduce_mean((loss_a + loss_b) * weights)  # 最终得到a2
